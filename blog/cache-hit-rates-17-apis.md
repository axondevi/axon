...
slug: cache-hit-rates-17-apis
description: Real data from 30 days of Axon traffic across 17 APIs. Which APIs are cache goldmines, which are impossible, and what that means for your agent's budget.
tags: [data, cache, ai-agents, economics]
published_at: 2026-04-25
reading_time: 7
---

# Cache hit rates across 17 APIs — what we learned

One of the earliest design bets we made with [Axon](https://axon.dev) was to put an aggressive cache in front of every upstream API. When the same agent (or a different agent!) asks the same question, we serve the cached response at **50% of list price** — faster for the user, pure margin for us, zero cost against the upstream.

The thesis was: *most agent traffic is more repeatable than you'd think.*

We've now run 30 days of production traffic across 17 APIs. Here's what actually happened, unvarnished. This is also [live on our public stats page](https://axon.dev/stats.html), updated every 5 minutes.
...
These numbers change as we add APIs, as upstream providers change, and as user traffic shifts. We publish them live at **[axon.dev/stats.html](https://axon.dev/stats.html)**. No login, no per-user data, just aggregate performance.
...
If you run an agent on Axon and want to contribute to these numbers — [$5 free credit here](https://axon.dev).
