...
description: Construa um agente IA em Python que usa dados brasileiros reais (CNPJ, CEP, FIPE) através do Nexus Inovation. Sem gerenciar múltiplas API keys.
category: brazil
...
Se quer ver o Nexus Inovation funcionando <strong>agora mesmo no browser</strong>, sem instalar Python,
...
- Roda com **uma única chave de API** (Nexus Inovation) — sem precisar se cadastrar em 10 serviços
...
- Uma conta no Nexus Inovation (criamos agora, leva 10 segundos)
...
## Passo 1 — Criando a conta Nexus Inovation
...
def call_axon(api: str, endpoint: str, params: dict = None):
    """Chama qualquer API através do Nexus Inovation com uma única chave."""
...
<Callout type="porque">
Observe que **não temos chave da BrasilAPI no código**. O Nexus Inovation cuida disso. Trocar pra outra API (digamos, Receita Federal paga) seria mudar só `"brasilapi"` pra `"receitaws"`. Zero retrabalho.
</Callout>
...
3. **Executamos a ferramenta** localmente (que bate no Nexus Inovation).
4. **Nexus Inovation bate na API real** (BrasilAPI, nesse caso — 100% gratuita, zero margem de custo pra você).
5. **Resultado volta pro LLM** como nova mensagem.
6. **LLM gera resposta final** em linguagem natural, baseada nos dados.

## Quanto custou?

Nexus Inovation retorna o header `x-axon-cost-usdc` em cada chamada. Pra ver:
...
Ou seja: **$1 dá pra rodar 125-330 perguntas**. Sua conta nova no Nexus Inovation com $0.50 grátis já dá pra testar bastante.

## Por que Nexus Inovation em vez de chamar BrasilAPI direto?

Nesse tutorial você poderia substituir `call_axon` por `requests.get("https://brasilapi.com.br/...")` e funcionaria. Então por que usar Nexus Inovation?

Porque em produção real:

1. **Você vai querer rate limiting** — BrasilAPI limita rápido. Nexus Inovation tem cache agressivo que reduz chamadas ao upstream em ~50-70%.
2. **Você vai querer mais APIs** — só a OpenAI já duplica o problema. Com 5+ APIs, gerenciar chaves vira pesadelo. Com Nexus Inovation: 1 chave.
3. **Você vai querer métricas** — Nexus Inovation dá dashboard com custo, latência, cache hit rate. Grátis.
...
Todas disponíveis no `brasilapi` do Nexus Inovation. É copiar uma das funções existentes e trocar o endpoint.
