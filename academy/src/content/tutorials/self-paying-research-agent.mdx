...
subtitle: A working agent that searches, scrapes, and synthesizes — paying its own USDC per call. No SerpAPI signup. No Firecrawl subscription.
description: Ship a real autonomous research agent using Nexus Inovation + Vercel AI SDK. One API key, USDC wallet, 20 lines of code.
category: agents
...
tags: [tutorial, ai-agents, vercel-ai-sdk, x402, self-paying]
featured: true
relatedTutorials:
  - primeiro-agente-apis-brasileiras
---
...
<Callout type="porque">
This tutorial showcases the **self-paying agent** pattern that Nexus Inovation was built for. Each tool call is atomically debited in USDC from the agent's own wallet. The agent literally cannot spend more than you deposited.
</Callout>
...
- `AXON_KEY` — [sign up for free $0.50 credit](https://axon-5zf.pages.dev) to get one
- `OPENAI_API_KEY` — for the LLM driving the agent

## The whole agent

```ts
import { Nexus Inovation } from '@axon/client';
...
**1. Nexus Inovation wraps paid APIs.** `axonTool(axon, 'serpapi', 'search', ...)` creates a Vercel AI SDK tool that, when called, hits Nexus Inovation → SerpAPI → back. You never see a SerpAPI key. The cost is debited from your Nexus Inovation wallet in USDC.
...
**3. The wallet makes it autonomous.** Every call debits USDC atomically. If the agent tries to spend more than you deposited, Nexus Inovation returns `402 insufficient_funds` and the agent stops. You set a budget, the agent works within it.

<Callout type="dica">
For tighter budget control, use Nexus Inovation's policy engine: `max_request_cost_micro`, `daily_budget_micro`, per-API caps. Your agent can rampage within limits you set.
</Callout>
...
Nexus Inovation returns `x-axon-cost-usdc` on every call. Typical run:
...
- Swap `gpt-4o-mini` for `claude-sonnet-4-6` via `@ai-sdk/anthropic` — Nexus Inovation bills both through the same wallet
...
Cloneable repo at [`axondevi/axon` → `templates/research-agent-ts`](https://github.com/axondevi/axon/tree/main/templates/research-agent-ts). Runs out of the box.

If you build something on this, we'd love to see it — DM [@axondevia](https://twitter.com/axondevia).
