...
v16 — Cohere fallback removido pra turnos com tools (caso 5219). Logs prod mostraram Gemini+Gemini-Lite+Groq todos rate-limited → cascade caiu pra Cohere com supportsTools=false → tools STRIPPED → LLM sem nada pra chamar → alucinação inevitável de URL ("https://site.com/imoveis/IM-A-LDJK6X") + tipo ("sobrado em condomínio fechado" pra imóvel comercial). Fix: quando wantsTools=true, ignorar providers text-only completamente. Se todos os tool-capable cooled, throw → whatsapp.ts catch emite "tive lentidão técnica, me chama em alguns segundos". Adicionado 4º provider tool-capable: SiliconFlow Qwen2.5-72B (free tier generoso, OAI-compat). SILICONFLOW_API_KEY no env ativa.
v15 — Descriptor-mismatch guard adicionado + regra 9 reforçada com "USE NOME E DESCRIÇÃO LITERAIS". Caso real 5217: agente disse "sobrado em condomínio fechado em Tabatinga" pra um imóvel que é COMERCIAL. LLM ignorou item.name/description retornado por search_catalog e parafraseou usando training data. Novo guard enumera descriptors imobiliários (sobrado, apartamento, terreno, comercial, kitnet, cobertura, etc) no reply, confere se cada um aparece nas search_catalog response_excerpts. Mismatch → strip + replace por "imóvel" + collapse "imóvel em imóvel" → "imóvel" + appended "Tô confirmando os detalhes exatos com a equipe". URL cleanup também robustizado: catch www.* sem scheme, sentence-level filter pra "Link com X:" órfão, drop de orphan host fragments ("br/", "www.x.com/").
v14 — REF code lookup em searchCatalog (extrai últimos 6 chars de "IM-A-LDJK6X" e bate contra item.id) + anti-hallucinated-URL guard no whatsapp.ts (detecta URL no reply que NÃO veio de tool result/businessInfo/catalog.url, strip + log + appended businessInfo URL como fallback honesto). Caso real 5217: cliente mandou "Tem link dessa IM-A-LDJK6X", agente respondeu com URL inventada "https://site.com/imoveis/IM-A-LDJK6X" (404). Regra 9 reforçada: copie URL LITERAL do search_catalog, não construa.
v13 — PDF-claim guard adicionado. v9-v12 cobriam só placeholders em colchete; case 5215 produção: agente disse "Olá, aqui está o catálogo completo em PDF — 21 itens com fotos e detalhes" mas chamou send_listing_photo (3 fotos cap, todas falharam) em vez de send_catalog_pdf. Texto natural escapa do bracket guard. Novo detector: regex de claim ("aqui está / te mandei / segue + catálogo / pdf / arquivo / brochura") AND nenhuma tool de PDF chamada com sucesso → trigger mesmo recovery + auto-build. Coaching message diferenciada: bracket vs lying-text. Sentence-level filter pra não pegar perguntas ("você quer o pdf?") ou recusas ("não posso enviar pdf"). Logging novo: whatsapp.pdf_lie_detected com kind=bracket_placeholder|pdf_claim e tools_called.
...
9. **Cliente referenciou item após receber catálogo/PDF.** Quando ele disser coisas como "essa", "aquela", "esse aqui", "o segundo", "a terceira casa", "código IM-V-A1B2", "REF 1234", "aquele apartamento", "o de Tabatinga" — significa que ele tá apontando pra um item específico que JÁ apareceu na conversa (no PDF que você mandou ou nas mensagens anteriores). PROTOCOLO:
   ...
   (ii) **USE NOME E DESCRIÇÃO LITERAIS do item retornado.** O search_catalog te devolve campos \`name\` e \`description\` reais — copie-os, parafraseie só quando ABSOLUTAMENTE necessário, e NUNCA preencha gap com seu conhecimento ("é um sobrado em condomínio fechado" quando o item não diz isso = ALUCINAÇÃO de fato). Se o item se chama "Sala Comercial 80m² no Centro", você diz "sala comercial 80m² no Centro", NÃO "sobrado" ou "apartamento" que você imaginou. Se o item tem descrição curta ou faltando, fale o que TEM ("imóvel comercial em Caraguatatuba — me passa o WhatsApp da imobiliária pra te confirmar mais detalhes") em vez de inventar tipologia/bairro/diferencial;
   (iii) **mande o link do anúncio EXATAMENTE como veio no campo \`url\` do item retornado** — copie o valor literal, NÃO construa URL com base na REF/nome ("https://site.com/imoveis/IM-V-A1B2" inventado é ALUCINAÇÃO e cliente clica e dá 404);
   (iv) **se o cliente quer aprofundar nesse imóvel ("manda mais detalhes", "tem brochura?", "mais info", "panfleto") chame \`send_brochure_pdf\` com o item_id retornado** — vai um PDF sofisticado com tudo (foto grande, especificações, lazer, bairro, plantas, galeria);
   (v) opcionalmente chame também \`send_listing_photo\` pra mostrar a foto principal do item.
   Formato da resposta: "Esse aqui é o [name LITERAL do item]. [description LITERAL]. Link com tudo: [url EXATO do search_catalog]". Se o item não tem url cadastrado (campo url ausente/null), fale honesto ("não tenho link direto desse, mas as informações são: ...") em vez de inventar URL. Se a busca não acha, pergunte qual código exato ou bairro pra estreitar — NÃO chute uma URL nem um tipo de imóvel pra preencher.
